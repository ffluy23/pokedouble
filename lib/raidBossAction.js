...
export async function executeBossAction(roomId, data, entries, currentOrder, extraUpdate = {}) {
  ...
  if (bossAI.checkPhase2Enter) {
    ...
    if (p2) {
      ...
      if (p2._clearGrudges) {
        PLAYER_SLOTS.forEach(s => {
          const p = entries[s]?.[data[`${s}_active_idx`] ?? 0]
          if (p) p.grudge = false
        })
      }    
    }     
  ...
  if (data.boss_name === "조로아크") {
    nextState = {
      ...(data.boss_state ?? {}),
      ...nextState,
      shadowClawTotalDmg: data.boss_state?.shadowClawTotalDmg ?? nextState.shadowClawTotalDmg ?? 0,
      stolenMoves:        data.boss_state?.stolenMoves        ?? nextState.stolenMoves        ?? {},
