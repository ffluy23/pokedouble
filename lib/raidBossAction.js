...
import { getDealCheckLog } from "./bosses/garbodor.js"  // ← [부식] 딜체크 로그
import { processZoroarkCommand, processZoroarkEot } from "./zoroarkcommands.js"
...
function calcBossDamage(data, moveName, targetPkmn, diceOverride = null) {
  ...
  // ── [심안] 특성: 노말·격투는 고스트에게도 명중 ──────────────
  if (data.boss_name === "다투곰" && ["노말", "격투"].includes(moveInfo.type)) {
...
function processCorruptionBlast(targetSlot, data, entries, logEntries) {
  ...
  if (mult === 0) {
    ...
    if (pkmn.hp <= 0) {
      logEntries.push(makeLog("faint", `${tName}${josa(tName, "은는")} 쓰러졌다!`, { slot: realTarget }))
    }
  }
...
function processBossAttack(moveName, targetSlot, isAoe, data, entries, logEntries, bossName) {
  ...
  if (isAoe) {
    ...
    if (data.sync_active && alive.length > 1) {
      ...
      logEntries.push(makeLog("sync", ""))
      logEntries.push(makeLog("after_hit",
        `💠 싱크로나이즈! 광역 공격을 ${alive.length}명이 함께 버텼다! (×0.75 후 ${alive.length}등분, 각 ${share})`
      ))
    ...
    for (const slot of Object.keys(damages)) {
      ...
      if ((defender?.lightScreen ?? 0) > 0 && damages[slot] > 0) {
        ...
        logEntries.push(makeLog("normal",
          `${defender.name}${josa(defender.name, "은는")} 이상한 힘으로 피해를 줄였다!`
        ))
      }
    }
...
// ── 반동 피해 ─────────────────────────────────────────────
    if (moveInfo?.recoil && damage > 0) {
      const recoilDmg = Math.max(1, Math.floor(damage * moveInfo.recoil))
      data.boss_current_hp = Math.max(0, (data.boss_current_hp ?? 0) - recoilDmg)
      logEntries.push(makeLog("normal",
        `${bossName}은(는) 반동으로 ${recoilDmg} 데미지를 입었다!`
      ))
...
export async function executeBossAction(roomId, data, entries, currentOrder, extraUpdate = {}) {
  ...
  const isMalamarCommand = command?.startsWith("malamar_")  // ── [칼라마네로] ──
  const isUrsalunaCmd    = command === "ursaluna_fury"

  if ((data.boss_volatile?.confused ?? 0) > 0) {
    data.boss_volatile = {
      ...data.boss_volatile,
      confused: data.boss_volatile.confused - 1
    }
  ...
  const isStatusBlocked =
    ...
    !isCorruptionCmd &&
    !isDelphoxCommand &&
    !isMalamarCommand &&  // ── [칼라마네로] 상태이상 차단 예외 ──
  ...
  } else if (command === "ursaluna_fury") {
    logEntries.push(makeLog("normal", `${bossName}은(는) 자신의 몸을 강하게 내리친다!`))
    logEntries.push(makeLog("normal", `${bossName}은(는) 고통을 무시하고 있다.`))
    const selfDmg = Math.max(1, Math.floor((data.boss_max_hp ?? 1) * 0.10))
    data.boss_current_hp = Math.max(0, (data.boss_current_hp ?? 0) - selfDmg)
    logEntries.push(makeLog("hp", "", { slot: "boss", hp: data.boss_current_hp, maxHp: data.boss_max_hp }))
    const bRank = data.boss_rank ?? defaultRanks()
    data.boss_rank = { ...bRank, atk: 4, atkTurns: 99 }
    logEntries.push(makeLog("normal", `${bossName}의 공격 랭크가 최대로 올라갔다!`))
  ...
  // ════════════════════════════════════════════════════════════════
    ...
    if (targetSlot) {
      ...
      if (pkmn && pkmn.hp > 0) {
        ...
        logEntries.push(makeLog("normal", `${pkmn.name}${josa(pkmn.name, "은는")} 현혹되었다!`))
      }
    }

  } else if (command === "malamar_collect") {
    // 나쁜음모: 수집 완료 알림 (실제 기록은 raidUseMove.js에서)
    if (commandLog) logEntries.push(makeLog("normal", commandLog))
    ...
    if (total > 0) {
      logEntries.push(makeLog("normal", `칼라마네로는 기분 나쁘게 웃고 있다...`))
    }
    ...
    if (commandLog) logEntries.push(makeLog("normal", commandLog))
    if (moveName === "사이코쇼크") {
      // 사이코쇼크: bossMoves에 위력 75로 정의되어 있어야 함
  ...
  // ════════════════════════════════════════════════════════════════
  } else if (command?.startsWith("zoroark_")) {
  const zResult = processZoroarkCommand(command, decision, data, entries, logEntries)
  if (zResult?.nextState) nextState = zResult.nextState
  ...
  // ── 독침붕 전멸 체크 ────────────────────────────────────────────
 if (command !== "direct" && command !== "recharge" && command !== "summon" && command !== "kangaskhan_dual" && !isNuriCommand && !isCorruptionCmd && !isMalamarCommand && !command?.startsWith("zoroark_")) {
  ...
  if (bossAI.processUnitSwap && !bossJustDied) {
    ...
    if (swapResult) {
      for (const text of swapResult.logs) {
        logEntries.push(makeLog("normal", text))
      }
      nextState = swapResult.nextState
      if (bossAI.applyInheritedRanks) {
        bossAI.applyInheritedRanks(data, swapResult.nextState.inheritedRanks)
      }
    }
  }

  // ── 마폭시 EOT (리플렉터/봉인 턴 감소) ─────────────────────────
  if (!bossJustDied && bossAI.processDelphoxEot) {
    processDelphoxEot(data, entries, logEntries)
  }

  if (!bossJustDied && data.boss_name === "조로아크") {
    processZoroarkEot(data, entries, logEntries)
  }
