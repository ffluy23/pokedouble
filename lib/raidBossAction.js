...
import { processNinjaskPhase3, processNinjaskEot } from "./bosses/ninjask.js"
...
export async function executeBossAction(roomId, data, entries, currentOrder, extraUpdate = {}) {
  ...
  // ════════════════════════════════════════════════════════════════
    ...
    if (total > 0) {
      logEntries.push(makeLog("normal", `기억된 기술: ${unique}종류 (총 ${total}회 관찰)`))
    }
  ...
  // ── 독침붕 전멸 체크 ────────────────────────────────────────────
  if (command !== "direct" && command !== "recharge" && command !== "summon" && command !== "kangaskhan_dual" && !isNuriCommand && !isCorruptionCmd && !isMalamarCommand && !isZoroarkCommand) {
 ...
 // ── 3페이즈 진입 체크 ───────────────────────────────────────────
  ...
  await processNinjaskPhase3(roomId, data, entries, logEntries, db)
}
...
// ── 껍질몬 EOT ────────────────────────────────────────────
if (!bossJustDied && data.boss_name === "껍질몬") {
  processNinjaskEot(data, entries, logEntries)
}
