...
function applyCorruption(targetSlot, data, entries, logEntries) {
  ...
  // boss_state 에 부식 대상 기록 (decideBossMove 에서 이미 nextState 에 설정하지만
  // 혹시 applyCorruption 이 직접 호출되는 경우를 위한 보험)
...
function processCorruptionBlast(targetSlot, data, entries, logEntries) {
  ...
  if (dmgSoFar >= CORRUPTION_THRESHOLD) {
    ...
    // 더스트나가 분노했다! 는 딜성공 시 출력 안 함 (실패 후 딜 때만)
    return
  }
...
function processBossAttack(moveName, targetSlot, isAoe, data, entries, logEntries, bossName) {
  ...
  if (isAoe) {
    ...
    for (const s of alive) {
      ...
      // 베놈쇼크: 독 상태이상 대상 1.2배 ([부식]과 무관, 일반 status === "독")
...
// ── [마폭시] 전용 커맨드 ─────────────────────────────────────────
  ...
  // ── 독침붕 전멸 체크 ────────────────────────────────────────────
  if (command !== "direct" && command !== "recharge" && command !== "summon" && command !== "kangaskhan_dual" && !isNuriCommand && !isCorruptionCmd) {
