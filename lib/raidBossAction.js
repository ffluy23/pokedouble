...
// ════════════════════════════════════════════════════════════════════
//  Roster 추상화 + 레거시(p1_entry) 완전 호환
//
//  우선순위:
//  1. roster[uid].entry 있으면 → 그걸 씀 (교체/데미지 반영 최신값)
//  2. roster 없는데 p1_entry 이미 있으면 → 건드리지 않음 (레거시)
//  3. 둘 다 없으면 → 빈 배열
// ════════════════════════════════════════════════════════════════════
...
export function hydrateSlotData(data) {
  ...
  for (const slot of PLAYER_SLOTS) {
    ...
    if (member?.entry?.length > 0) {
      // roster에 유효한 entry → 최신값 사용
      data[`${slot}_entry`]      = JSON.parse(JSON.stringify(member.entry))
      data[`${slot}_active_idx`] = member.active_idx ?? 0
    } else if (data[`${slot}_entry`]?.length > 0) {
      // 레거시: p1_entry 등이 이미 있으면 건드리지 않음
    } else {
      // 둘 다 없으면 빈 배열
      data[`${slot}_entry`]      = []
...
export function getAlivePlayers(data, entries) {
  ...
  return PLAYER_SLOTS.filter(s => {
    // roster 기반: uid 없는 슬롯 제외
    if (hasActiveSlots && !activeSlots[s]) return false
    // 레거시: entry 자체가 없으면 제외
    if (!entries[s] || entries[s].length === 0) return false
...
export function checkRaidWin(entries, bossHp, data) {
  ...
  const allDead = PLAYER_SLOTS.every(s => {
    if (hasActiveSlots && !activeSlots[s]) return true   // roster: 빈 슬롯 → dead 취급
    if (!entries[s] || entries[s].length === 0) return true  // entry 없으면 → dead 취급
...
function getTauntSelfTarget(data, entries) {
  const taunters = PLAYER_SLOTS.filter(s => {
    if (!(data.active_slots ?? {})[s] && Object.values(data.active_slots ?? {}).some(v => !!v)) return false
    const idx  = data[`${s}_active_idx`] ?? 0
    const pkmn = entries[s]?.[idx]
...
function processBossAttack(moveName, targetSlot, isAoe, data, entries, logEntries, bossName) {
  ...
  if (isAoe) {
    ...
    const _gutsPower = (bossMoves[moveName]?.gutsLike &&
      ...
      ? (bossMoves[moveName].power ?? 70) * 2 : null
...
async function fetchBeedrilStats(roomId) {
  ...
  return {
    attack: base.attack ?? 3, defense: base.defense ?? 3, speed: base.speed ?? 3,
    hp: base.hp ?? 60, maxHp: base.hp ?? 60, moves: base.moves ?? [],
    type: base.type ?? ["벌레", "독"], portrait: base.portrait ?? null,
    name: base.name ?? "독침붕", ranks: defaultRanks(), wasHealed: false,
  }
}
...
function checkBeedrilDeath(data, nextState) {
  ...
  return { allDead: true, nextState: { ...nextState, step: "recharge", beedrillKillCount: newKillCount } }
}
...
export async function executeBossAction(roomId, data, entries, currentOrder, extraUpdate = {}) {
  ...
  if (data.boss_volatile?.flinch) {
    ...
    await db.collection("raid").doc(roomId).update({
      ...dehydrateSlotData(data, entries),
      ...Object.fromEntries(PLAYER_SLOTS.map(s => [`${s}_entry`,      entries[s] ?? []])),
      ...Object.fromEntries(PLAYER_SLOTS.map(s => [`${s}_active_idx`, data[`${s}_active_idx`] ?? 0])),
  ...
  const isStatusBlocked =
    ...
    !isNuriCommand && !isCorruptionCmd && !isDelphoxCommand &&
  ...
  data.sync_used    = false   // ← 원래 있던 거
  data.sync_active  = false   // ← 추가
  data.sync_request = null    // ← 추가
  ...
  if (!bossJustDied && data.boss_name === "칼라마네로") {
    for (const s of PLAYER_SLOTS) {
      ...
      if (hasActiveSlots && !activeSlots[s]) continue
      const idx  = data[`${s}_active_idx`] ?? 0
      const pkmn = entries[s]?.[idx]
      if (!pkmn || pkmn.hp <= 0) continue
  ...
  const update = {
    ...rosterPatch,
    // ── 레거시 p1_entry 필드도 항상 최신값으로 저장 ──────────────
    // patchLegacySlots가 다음 턴에 p1_entry를 읽어도 데미지/교체 반영된 값
    ...Object.fromEntries(PLAYER_SLOTS.map(s => [`${s}_entry`,      entries[s] ?? []])),
    ...Object.fromEntries(PLAYER_SLOTS.map(s => [`${s}_active_idx`, data[`${s}_active_idx`] ?? 0])),
