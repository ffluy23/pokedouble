...
function getBrandedSlot(data) {
  return data.boss_state?.brandedSlot ?? null
}
...
function processBrandBlast(data, entries, logEntries) {
  ...
  // 블래키 최우선 체크
  const damagesMap = { [targetSlot]: BRAND_BLAST_DMG }
  activateUmbreon(damagesMap, data, entries, logEntries)
  const finalDmg = damagesMap[targetSlot]
  ...
  if (finalDmg > 0) {
    ...
    if (pkmn.hp <= 0)
      logEntries.push(makeLog("faint", `${pkmn.name}${josa(pkmn.name, "은는")} 쓰러졌다!`, { slot: targetSlot }))
  }
...
function _attackTarget(moveName, targetSlot, data, entries, logEntries) {
  ...
  // 낙인 +10% (슬롯 기준)
  if (data.boss_state?.brandedSlot === targetSlot) {
  ...
  // ── 블래키 최우선 체크 ──────────────────────────────────────────
  const damagesMap = { [targetSlot]: damage }
  activateUmbreon(damagesMap, data, entries, logEntries)
  const finalDmg = damagesMap[targetSlot]
...
export function checkTrickeryRedirect(attackerSlot, targetSlot, data, entries) {
  ...
  if (data.boss_state?.brandedSlot !== attackerSlot) return { redirected: false }
