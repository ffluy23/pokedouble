...
function getGrudgeSlots(data, entries) {
  return PLAYER_SLOTS.filter(s => {
    ...
    return p && p.hp > 0 && data[`${s}_grudge`] === true  // ← 슬롯에 저장
  })
}

function getNonGrudgeAlive(data, entries) {
  return getAlivePlayers(data, entries).filter(s => !data[`${s}_grudge`])
}
...
function applyGrudge(targetSlot, data, entries, logEntries) {
  ...
  data[`${targetSlot}_grudge`] = true
 logEntries.push(makeLog("normal",
  `${BOSS_NAME}${josa(BOSS_NAME, "이가")} ${pkmn.name}에게 원한을 품었다!`
))
  return true
}
...
function clearAllGrudges(data, entries, logEntries) {
  for (const s of PLAYER_SLOTS) {
    data[`${s}_grudge`] = false  // ← 슬롯 기준으로 저장
  }
...
function processRevenge(data, entries, logEntries) {
  ...
  const revengeDmg = Math.max(1, totalDmg * 3)

  for (const slot of grudgeSlots) {
    const pkmn = activePkmn(slot, data, entries)
    if (!pkmn || pkmn.hp <= 0) continue

    logEntries.push(makeLog("normal", `누적된 원한이 폭발했다!`))

    const damagesMap = { [slot]: revengeDmg }
    activateUmbreon(damagesMap, data, entries, logEntries)
    const finalDmg = damagesMap[slot] ?? revengeDmg
...
function stealBestMoves(data, entries, logEntries) {
  ...
  // ← 핵심: data.boss_state 전체를 스프레드해서 기존 값 유지
  data.boss_state = {
    ...(data.boss_state ?? {}),
    stolenMoves: storedMoves,
  }

  logEntries.push(makeLog("normal", "조로아크는 만족스럽게 웃고 있다!"))
}
...
function useStolenMove(data, entries, logEntries) {
 ...
 const nonGrudge = alive.filter(s => !data[`${s}_grudge`])
...
function decidePhase3(data, entries) {
  ...
  switch (p3Step) {
    case 1: {
      ...
      return {
        command: hasScreen
          ? (useCurse ? "zoroark_p3_breaker_curse_taunt" : "zoroark_p3_breaker_taunt")
          : (useCurse ? "zoroark_p3_curse_taunt"         : "zoroark_p3_taunt"),
        msgIndex,
...
export function processZoroarkCommand(command, decision, data, entries, logEntries) {
  ...
  switch (command) {
    ...
    case "zoroark_steal": {
      ...
      logEntries.push(makeLog("normal",
        `${illusionName}${josa(illusionName, "이가")} 무언가 훔쳐본 것 같다...!`
      ))
      break
    }
    ...
    case "zoroark_p3_breaker_curse_taunt": {
  processBreaker(data, entries, logEntries)
  processZoroarkCurse(data, entries, logEntries)
  processTaunt(data, entries, logEntries, decision.msgIndex ?? 0)
  break
}
case "zoroark_p3_breaker_taunt": {
  processBreaker(data, entries, logEntries)
  processTaunt(data, entries, logEntries, decision.msgIndex ?? 0)
  break
}
