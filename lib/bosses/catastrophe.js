...
export function checkPhase2Enter(data, nextState) {
  const phase = nextState.phase ?? 1
  if (phase !== 1) return null
  if ((nextState.totalDmgTaken ?? 0) <= 900) return null
  return {
...
return {
  command: "catastro_phase1_attack", log: null, moveName, targetSlot, nextState,
}
}
...
function decidePhase2(state, data, entries, PLAYER_SLOTS) {
 ...
 if (step === 0) {
  const destroyed = state.partDestroyed ?? {}
const availableMoves = PHASE2_MOVES.filter(mv => !destroyed[PART_MAP[mv]?.part])
if (availableMoves.length === 0) {
  return { command: "idle", log: "...", nextState: state }
}
const moveName = rand(availableMoves)
