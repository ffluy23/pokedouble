...
export function decideBossMove(data, entries, playerSlots) {
  ...
  return {
    ...
    targetSlot,
    log:        isPsycho
      ? `${bossName}${bossName.endsWith("로") ? "가" : "이"} 사이코쇼크를 날렸다! (score: ${score.toFixed(2)})`
      : `${bossName}${bossName.endsWith("로") ? "가" : "이"} 바꿔치기! 기억한 기술 [${moveName}]을 사용한다! (score: ${score.toFixed(2)})`,
    nextState: { ...bossState },
  }
}
...
export function checkPhase2Enter(data, nextState, command) {
  ...
  return {
    logs: [
      `${bossName}의 눈빛이 변했다!`,
      `${bossName}이(가) 2페이즈로 돌입했다! 저장된 기억으로 반격한다!`,
    ],
