...
function simulateValue(moveName, moveInfo, data, entries, profiles) {
  ...
  if (category === "attack") {
    ...
    const baseDmgScore = (power + bossAtk * 4) / 150   // 정규화
    score = baseDmgScore * targetCount * 0.5

    // 적이 HP 낮을수록 킬 기대값 ↑
    score += (1 - minHpRatio) * 0.3

    // 적이 많이 쓰러져서 1명이면 단타 우선
...
function selectBestMove(data, entries, storedMovesFlat) {
  ...
  // 타겟 결정: HP 가장 낮은 생존자
  if (bestMove !== "사이코쇼크") {
    const moveInfo = storedMovesFlat.find(m => m.moveName === bestMove)?.moveInfo ?? {}
    const isAoe    = !!(moveInfo?.aoe || moveInfo?.aoeEnemy)
    if (!isAoe && alive.length > 0) {
      let minHp  = Infinity
      let minSlot = alive[0]
      for (const s of alive) {
        const idx  = data[`${s}_active_idx`] ?? 0
    ...
    // 사이코쇼크: HP 가장 낮은 생존자
    if (alive.length > 0) {
      let minHp = Infinity; let minSlot = alive[0]
      for (const s of alive) {
        const idx  = data[`${s}_active_idx`] ?? 0
        const pkmn = entries[s]?.[idx]
        if (pkmn && pkmn.hp < minHp) { minHp = pkmn.hp; minSlot = s }
...
function buildStoredMovesFlat(storedMoves) {
  ...
  for (const [slot, moveList] of Object.entries(storedMoves)) {
    for (const entry of moveList) {
      ...
      if (moveInfo) flat.push({ moveName: entry.moveName, moveInfo, category: entry.category })
    }
  }
  return flat
}
...
export function decideBossMove(data, entries, playerSlots) {
  ...
  return {
    ...
    log:        isPsycho
      ...
      : `${bossName}${bossName.endsWith("로") ? "가" : "이"} 바꿔치기! 기억한 기술 [${moveName}]을 사용한다!`,
    nextState: { ...bossState },
  }
}
