...
function applyRankChanges(r, self, target, moveName, logEntries) {
  ...
  function applyOne(obj, key, delta, name, isTarget = false) {
    ...
    if (delta > 0) {
      ...
      if (isTarget) target.hadRankDropThisTurn = true
    }
  }
...
function calcDamage(atk, moveName, def, powerOverride = null, atkStatOverride = null, diceOverride = null, ignoreDefRank = false, forceCrit = false) {
  ...
  const critRate = forceCrit ? 100 : Math.min(100, atkStat * 2 + (move.highCrit ? 3 : 0))
...
// ── 장판 데미지 공통 헬퍼 (roar 강제교체 / 일반 교체 공통 사용) ──
function applyHazardDamage(slot, newPkmn, data, logEntries) {
  const team = teamOf(slot)

  // 스텔스록
  const srKey = `field_${team}_stealth_rock`
  if ((data[srKey] ?? 0) > 0 && newPkmn.hp > 0) {
    const dmg = Math.max(1, Math.floor((newPkmn.maxHp ?? newPkmn.hp) * 0.125))
    newPkmn.hp = Math.max(0, newPkmn.hp - dmg)
    logEntries.push(makeLog("normal", `스텔스록이 ${newPkmn.name}${josa(newPkmn.name, "을를")} 공격했다! (-${dmg})`))
    logEntries.push(makeLog("hp", "", { slot, hp: newPkmn.hp, maxHp: newPkmn.maxHp }))
    if (newPkmn.hp <= 0) logEntries.push(makeLog("faint", `${newPkmn.name}${josa(newPkmn.name, "은는")} 쓰러졌다!`, { slot }))
  }

  // 독압정
  const tsKey = `field_${team}_toxic_spikes`
  const tsVal = data[tsKey] ?? 0
  if (tsVal > 0 && newPkmn.hp > 0 && !newPkmn.status) {
    const pkmnTypes = Array.isArray(newPkmn.type) ? newPkmn.type : [newPkmn.type]
    if (!pkmnTypes.includes("비행") && !pkmnTypes.includes("강철") && !pkmnTypes.includes("독")) {
      applyStatus(newPkmn, "독").forEach(m => logEntries.push(makeLog("normal", m)))
    }
  }
}
...
function handleSpecialNonAttack(moveInfo, moveName, myPkmn, mySlot, tSlots, entries, data, logEntries) {
  ...
  if (moveInfo.effect?.moonlight) {
    ...
    if (weather === "쾌청") {
      healRate = 0.25
    } else if (weather === "비" || weather === "싸라기눈" || weather === "모래바람") {
      healRate = 0.18
    } else {
      healRate = 0.22
    }
  ...
  // ── 알낳기 / 우유마시기: 자신 22%, 아군 18% ─────────────────
  if (moveInfo.allyHeal !== undefined && moveInfo.effect?.heal) {
    if (tSlots.length > 0) {
      const tSlot = tSlots[0]
      const tIdx  = data[`${tSlot}_active_idx`] ?? 0
      const tPkmn = entries[tSlot][tIdx]
      if (tPkmn && tPkmn.hp > 0 && teamOf(tSlot) === teamOf(mySlot) && tSlot !== mySlot) {
        if ((tPkmn.healBlocked ?? 0) > 0) {
          logEntries.push(makeLog("normal", `${tPkmn.name}${josa(tPkmn.name, "의")} 회복이 봉인되어 있다!`))
        } else {
          const allyHeal = Math.max(1, Math.floor((tPkmn.maxHp ?? tPkmn.hp) * moveInfo.allyHeal))
          tPkmn.hp = Math.min(tPkmn.maxHp ?? tPkmn.hp, tPkmn.hp + allyHeal)
          logEntries.push(makeLog("hp", `${tPkmn.name}${josa(tPkmn.name, "은는")} HP를 회복했다! (+${allyHeal})`, { slot: tSlot, hp: tPkmn.hp, maxHp: tPkmn.maxHp }))
        }
        return { handled: true }
      }
    }
    ...
    if ((myPkmn.healBlocked ?? 0) > 0) {
      logEntries.push(makeLog("normal", `${myPkmn.name}${josa(myPkmn.name, "의")} 회복이 봉인되어 있다!`))
    } else {
  ...
  if (moveInfo.roar) {
    ...
    if (benchAlive.length === 0) {
      ...
      const newPkmn  = tEntry[randIdx]
      tPkmn.seeded   = false
      logEntries.push(makeLog("normal", `${tPkmn.name}${josa(tPkmn.name, "은는")} 물러났다!`))
      logEntries.push(makeLog("normal", `${newPkmn.name}${josa(newPkmn.name, "이가")} 나왔다!`))
      data[`${tSlot}_active_idx`] = randIdx

      // [수정 2] 날려버리기로 강제교체된 포켓몬에도 장판 데미지 적용
      applyHazardDamage(tSlot, newPkmn, data, logEntries)
    }
    return { handled: true }
  }
...
function handleSpecialAttack(moveInfo, moveName, myPkmn, mySlot, tSlot, tPkmn, entries, data, logEntries) {
  ...
  // ── [수정 3] 킬러스핀: rapidSpin + aoeEnemy 조합에서 effect.status(독) 적용 ──
  if (moveInfo.rapidSpin) {
    ...
    // 엉겨붙기 / 김밥말이 해제 (자신)
    if (myPkmn.wrapState) {
      myPkmn.wrapState = null
      logEntries.push(makeLog("normal", `${myPkmn.name}${josa(myPkmn.name, "은는")} 묶임에서 풀려났다!`))
    }
    // 장판 제거
    ...
    // effect.status 처리 (킬러스핀 독 부여)
    if (moveInfo.effect?.status && tPkmn.hp > 0) {
      const chance = moveInfo.effect.chance ?? 1
      if (Math.random() < chance) {
        applyStatus(tPkmn, moveInfo.effect.status).forEach(m => logEntries.push(makeLog("normal", m)))
      }
    }
    return { handled: true, damage }
  }
  ...
  if (moveInfo.saltWater) {
    const { hit, hitType } = calcHit(myPkmn, moveInfo, tPkmn)
    if (!hit) { missLog(hitType); return { handled: true, damage: 0 } }
    const hpRatio  = tPkmn.hp / (tPkmn.maxHp ?? tPkmn.hp)
    const power    = hpRatio <= 0.5 ? 80 : 65
    const { damage, multiplier, critical, minRoll, minDice } = calcDamage(myPkmn, moveName, tPkmn, power)
    if (multiplier === 0) { logEntries.push(makeLog("normal", `${tPkmn.name}에게는 효과가 없다…`)); return { handled: true, damage: 0 } }
    if (hpRatio <= 0.5) logEntries.push(makeLog("after_hit", `${tPkmn.name}${josa(tPkmn.name, "은는")} 체력이 절반 이하라 소금물이 더 아팠다!`))
    dealDamage(damage, multiplier, critical, tSlot, tPkmn, minRoll, minDice)
    return { handled: true, damage }
  }

  // ── [수정 1] 김밥말이(noSwitch:false)와 엉겨붙기(noSwitch:true) 분리 처리 ──
  if (moveInfo.wrap) {
    const { hit, hitType } = calcHit(myPkmn, moveInfo, tPkmn)
    if (!hit) { missLog(hitType); return { handled: true, damage: 0 } }
    const { damage, multiplier, critical, minRoll, minDice } = calcDamage(myPkmn, moveName, tPkmn)
    ...
    if (tPkmn.hp > 0) {
      if (!tPkmn.wrapState) {
        const turnsLeft = Math.floor(Math.random() * 2) + 4 // 4~5턴
        const noSwitch  = moveInfo.noSwitch === true  // 엉겨붙기만 true, 김밥말이는 false
        tPkmn.wrapState = {
          turnsLeft,
          attackerSlot: mySlot,
          moveName,
        ...
        // [수정 1] 메시지를 noSwitch 여부에 따라 분기
        const wrapMsg = noSwitch
          ? `${tPkmn.name}${josa(tPkmn.name, "은는")} 엉겨붙었다! 교체할 수 없다!`
          : `${tPkmn.name}${josa(tPkmn.name, "은는")} 휘감겼다!`
        logEntries.push(makeLog("normal", wrapMsg))
      }
    }
    return { handled: true, damage }
  }

  if (moveInfo.effect?.triAttack) {
    const { hit, hitType } = calcHit(myPkmn, moveInfo, tPkmn)
    if (!hit) { missLog(hitType); return { handled: true, damage: 0 } }
    const { damage, multiplier, critical, minRoll, minDice } = calcDamage(myPkmn, moveName, tPkmn)
    if (multiplier === 0) { logEntries.push(makeLog("normal", `${tPkmn.name}에게는 효과가 없다…`)); return { handled: true, damage: 0 } }
    dealDamage(damage, multiplier, critical, tSlot, tPkmn, minRoll, minDice)
    if (tPkmn.hp > 0 && Math.random() < (moveInfo.effect.chance ?? 0.2)) {
      const statuses = ["마비", "화상", "얼음"]
      const chosen   = statuses[Math.floor(Math.random() * statuses.length)]
      applyStatus(tPkmn, chosen).forEach(m => logEntries.push(makeLog("normal", m)))
    }
    return { handled: true, damage }
  }

  if (moveInfo.stomping) {
    const { hit, hitType } = calcHit(myPkmn, moveInfo, tPkmn)
    if (!hit) {
      myPkmn.missedLastTurn = true
      missLog(hitType)
      return { handled: true, damage: 0 }
    }
    const power = (myPkmn.missedLastTurn) ? 60 : 45
    myPkmn.missedLastTurn = false
    const { damage, multiplier, critical, minRoll, minDice } = calcDamage(myPkmn, moveName, tPkmn, power)
    if (multiplier === 0) { logEntries.push(makeLog("normal", `${tPkmn.name}에게는 효과가 없다…`)); return { handled: true, damage: 0 } }
    if (power > 45) logEntries.push(makeLog("after_hit", `분함이 폭발했다!`))
    dealDamage(damage, multiplier, critical, tSlot, tPkmn, minRoll, minDice)
    return { handled: true, damage }
  }
  ...
  if (moveInfo.vengeance) {
    const { hit, hitType } = calcHit(myPkmn, moveInfo, tPkmn)
    if (!hit) { missLog(hitType); return { handled: true, damage: 0 } }
    const hadRankDrop = myPkmn.hadRankDropThisTurn ?? false
    const power       = hadRankDrop ? 60 : 45
    const { damage, multiplier, critical, minRoll, minDice } = calcDamage(myPkmn, moveName, tPkmn, power)
    if (multiplier === 0) { logEntries.push(makeLog("normal", `${tPkmn.name}에게는 효과가 없다…`)); return { handled: true, damage: 0 } }
    if (hadRankDrop) logEntries.push(makeLog("after_hit", `분풀이 일격!`))
    dealDamage(damage, multiplier, critical, tSlot, tPkmn, minRoll, minDice)
    return { handled: true, damage }
  }

  if (moveInfo.burnOff) {
    const myTypes = Array.isArray(myPkmn.type) ? myPkmn.type : [myPkmn.type]
    if (!myTypes.includes("불")) {
      logEntries.push(makeLog("normal", `${myPkmn.name}${josa(myPkmn.name, "은는")} 불꽃 타입이 아니라 실패했다!`))
      return { handled: true, damage: 0 }
    }
    const { hit, hitType } = calcHit(myPkmn, moveInfo, tPkmn)
    if (!hit) { missLog(hitType); return { handled: true, damage: 0 } }
    ...
    if (multiplier === 0) { logEntries.push(makeLog("normal", `${tPkmn.name}에게는 효과가 없다…`)); return { handled: true, damage: 0 } }
    dealDamage(damage, multiplier, critical, tSlot, tPkmn, minRoll, minDice)
    if (tPkmn.hp > 0 && tPkmn.status === "얼음") {
      tPkmn.status = null
      logEntries.push(makeLog("normal", `${tPkmn.name}${josa(tPkmn.name, "의")} 얼음이 녹았다!`))
    }
    myPkmn._burnOffOrigType = myTypes
  ...
  if (moveInfo.alwaysCrit) {
    const { hit, hitType } = calcHit(myPkmn, moveInfo, tPkmn)
    if (!hit) { missLog(hitType); return { handled: true, damage: 0 } }
    const { damage, multiplier, critical, minRoll, minDice } = calcDamage(myPkmn, moveName, tPkmn, null, null, null, false, true)
    if (multiplier === 0) { logEntries.push(makeLog("normal", `${tPkmn.name}에게는 효과가 없다…`)); return { handled: true, damage: 0 } }
    dealDamage(damage, multiplier, true, tSlot, tPkmn, minRoll, minDice)
    return { handled: true, damage }
  }
...
// ── wrapState EOT 데미지 ──────────────────────────────────────────
async function applyWrapEot(entries, data, logEntries) {
  for (const tSlot of ALL_FS) {
    const tIdx  = data[`${tSlot}_active_idx`] ?? 0
    const tPkmn = entries[tSlot][tIdx]
    if (!tPkmn || !tPkmn.wrapState || tPkmn.hp <= 0) continue
    const ws = tPkmn.wrapState
    ws.turnsLeft--
    const dmg = Math.max(1, Math.floor((tPkmn.maxHp ?? tPkmn.hp) / 16))
    tPkmn.hp  = Math.max(0, tPkmn.hp - dmg)
    logEntries.push(makeLog("normal", `${tPkmn.name}${josa(tPkmn.name, "은는")} ${ws.moveName}으로 데미지를 입었다!`))
    logEntries.push(makeLog("hp", "", { slot: tSlot, hp: tPkmn.hp, maxHp: tPkmn.maxHp }))
    if (tPkmn.hp <= 0) {
      logEntries.push(makeLog("faint", `${tPkmn.name}${josa(tPkmn.name, "은는")} 쓰러졌다!`, { slot: tSlot }))
      tPkmn.wrapState = null
    } else if (ws.turnsLeft <= 0) {
      tPkmn.wrapState = null
      logEntries.push(makeLog("normal", `${tPkmn.name}${josa(tPkmn.name, "의")} 묶임이 풀렸다!`))
    }
  }
}
...
function handleTwoTurnAttack(myPkmn, mySlot, targetSlot, entries, data, logEntries, opts = {}) {
  ...
  if (wasDefending && moveName !== (myPkmn.ghostDiveMoveName ?? "고스트다이브")) {
...
export default async function handler(req, res) {
  ...
  if (!pre.blocked) {
    ...
    if (!conf.selfHit) {
      ...
      if (!moveInfo?.ghostDive) {
        for (const tSlot of tSlots) {
          const tIdx  = data[`${tSlot}_active_idx`] ?? 0
          const tPkmn = entries[tSlot][tIdx]
          if (!tPkmn || tPkmn.hp <= 0) continue
          if (tPkmn.defending) {
            tPkmn.defending = false; tPkmn.defendTurns = 0
            ...
            const winTeam = await finishTurn(roomRef, roomId, data, entries, logEntries, { ...assistUpdate, ...syncUpdate })
            return res.status(200).json({ ok: true, ...(winTeam ? { winTeam } : {}) })
          }
          tPkmn.defending = false; tPkmn.defendTurns = 0
        }
      ...
      if (!moveInfo?.power) {
        ...
        for (const tSlot of tSlots) {
          ...
          if (!hit) {
            logEntries.push(makeLog("normal", hitType === "evaded" ? `${tPkmn.name}에게는 맞지 않았다!` : `그러나 ${myPkmn.name}의 공격은 빗나갔다!`))
            if (moveInfo?.stomping) myPkmn.missedLastTurn = true
            if (moveInfo?.jumpKick) {
              const selfDmg = Math.max(1, Math.floor((myPkmn.maxHp ?? myPkmn.hp) * 0.25))
              myPkmn.hp = Math.max(0, myPkmn.hp - selfDmg)
              logEntries.push(makeLog("hp", `${myPkmn.name}${josa(myPkmn.name, "은는")} 반동으로 ${selfDmg} 데미지를 입었다!`, { slot: mySlot, hp: myPkmn.hp, maxHp: myPkmn.maxHp }))
          ...
          const forceCrit = !!(moveInfo?.alwaysCrit)
          let { damage, multiplier, critical, dice, minRoll, minDice } = calcDamage(myPkmn, moveData.name, tPkmn, powerOverride, null, aoeDice, moveInfo?.ignoreDefRank ?? false, forceCrit)
          ...
          if (moveInfo?.effect?.cureburn && tPkmn.hp > 0 && tPkmn.status === "화상") {
            tPkmn.status = null
            logEntries.push(makeLog("normal", `${tPkmn.name}${josa(tPkmn.name, "의")} 화상이 나았다!`))
          }
          if (moveInfo?.effect?.healBlock && tPkmn.hp > 0) {
            if ((tPkmn.healBlocked ?? 0) > 0) {
              logEntries.push(makeLog("normal", `${tPkmn.name}${josa(tPkmn.name, "은는")} 이미 회복봉인 상태다!`))
            } else {
              tPkmn.healBlocked = 3
  ...
  if (isEot) {
    ALL_FS.forEach(s => {
      ...
      pkmn.missedLastTurn = false
      pkmn.hadRankDropThisTurn = false
      if ((pkmn.burnOffTurns ?? 0) > 0) {
        pkmn.burnOffTurns--
        if (pkmn.burnOffTurns <= 0 && pkmn._burnOffOrigType) {
          pkmn.type = pkmn._burnOffOrigType
          pkmn._burnOffOrigType = null
          logEntries.push(makeLog("normal", `${pkmn.name}${josa(pkmn.name, "의")} 불꽃 타입이 돌아왔다!`))
        }
      }
    })
  }
  ...
  if (isEot) {
    ...
    await applyWrapEot(entries, data, eotLogEntries)
