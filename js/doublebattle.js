...
const API = "https://sonnettestdouble.vercel.app"
...
function updateSlotUI(slot, data) {
  ...
  if (nameEl) {
    ...
    const wrapTag      = pokemon.wrapState ? ` [묶임(${pokemon.wrapState.turnsLeft})]` : ""
    const myTeam = teamOf(slot)
    const screenTurns = data[`lightScreen_team${myTeam}`] ?? 0
    const screenTag = screenTurns > 0 ? ` [막(${screenTurns})]` : ""
    nameEl.innerText = (pokemon.name ?? "???") + statusTag + confusionTag + flyTag + digTag + wrapTag + screenTag
  }
...
function triggerAutoAction(data) {
  ...
  // 알낳기/우유마시기 자동처리: 아군 HP 낮으면 아군에게, 아니면 자신에게
  if (moveInfo.allyHeal !== undefined) {
    const allySlot = allyOf(mySlot)
    const allyIdx  = data[`${allySlot}_active_idx`] ?? 0
    const allyPkmn = data[`${allySlot}_entry`]?.[allyIdx]
    const allyHpRatio = allyPkmn && allyPkmn.hp > 0
...
function exitTargetMode() {
  ...
  if (hint) { hint.style.display = "none"; hint.innerText = "⚔ 공격할 상대를 클릭!" }
...
function enterTargetMode(idx, data, { targetsEnemy = true, targetsAlly = false, canSelf = false } = {}) {
  ...
  if (canSelf) {
    ...
    if (myArea) {
      myArea.classList.add("target-selectable")
      myArea.onclick = () => {
        playSound(SFX_BTN)
        const capturedIdx = pendingMoveIdx
        exitTargetMode()
    ...
    if (pkmn.hp <= 0) {
      ...
      if (isSpectator) {
        ...
        const canSwitch = (isFainted || forceSwitch || (myTurn && !actionDone))
          && !isDiving && !isFlying && !isDigging && !isWrapped
